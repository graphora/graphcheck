...
DEMO = ROOT / "examples" / "fraud-ring"
COMPOSE_PATH = DEMO / "compose.yml"
...
def test_example_profiles_match_local_quickstart_and_ci():
    profiles = yaml.safe_load((DEMO / "profiles.example.yml").read_text(encoding="utf-8"))
...
def test_default_checks_are_baseline_free_and_include_fraud_ring_suite():
    suites = _load_suite_inputs(DEMO / "checks", [])
    discovered = [
