...
TELEMETRY_DOC = ROOT / "docs" / "telemetry.md"
...
def _documented_allowlist() -> dict[str, object]:
    ...
    assert len(matches) == 1, "docs/telemetry.md must contain exactly one allowlist block"
...
def test_transparency_document_is_in_lockstep_with_outbound_allowlist():
    assert _documented_allowlist() == _code_allowlist(), (
        ...
        "telemetry allowlist block in docs/telemetry.md."
    )
