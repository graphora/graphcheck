...
**Source of truth:** the Pydantic models + loader at `src/graphcheck/contracts/check.py`. **Machine-valid example:** `tests/unit/contracts/fixtures/suite.valid.yml`.
...
- `docs/schemas/check.envelope.schema.json` (frozen) + `docs/schemas/check.schema.json` (generated, pack-versioned).
- `tests/unit/contracts/fixtures/suite.valid.yml` + `suite.invalid-*.yml`, and `tests/unit/contracts/test_check_validation.py`.
