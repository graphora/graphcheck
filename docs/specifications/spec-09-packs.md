...
truth for pack metadata. `docs/schemas/pack.schema.json` is generated from their
...
4. `docs/schemas/check.schema.json` is regenerated whenever the registry changes.
   `docs/schemas/check.envelope.schema.json` remains the frozen envelope schema;
   `docs/schemas/pack.schema.json` publishes the generated metadata contract.
...
in `docs/schemas/check.schema.json`.
...
The current test coverage for this spec lives in `tests/unit/engine/test_packs.py`,
`tests/unit/contracts/test_check_validation.py`, `tests/unit/engine/test_core_pack_compiler.py`,
`tests/unit/engine/test_pii_pack_runtime.py`, and the opt-in Neo4j integration suite.
